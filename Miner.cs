...
class Miner
{
    ...
    public string HashGrabber { get; set; }
    ...
    public void Go(object args)
    {
      ...
      //  HashGrabber = "Speed: " + (int)(hashspeed) + "KHash/Second!";
        //==================================Sumit if work done!=======================================
        if (fnonce != 0)
        {
    ...
    public void finish()
    {
        try
        {
            foreach (System.Diagnostics.Process proc in System.Diagnostics.Process.GetProcessesByName("stratumproxy"))
            {
                proc.Kill();
            }
        }
